...
class Player:
    ...
    def place_bet(self, true_count):
        true_count = round(true_count, 2)  # Ensure count updates correctly
        if true_count >= 2:
            self.current_bet = min(int(self.current_bet * 2.5), self.chips // 3)  # More aggressive betting
        elif true_count <= 0:
...
# Function for AI to play optimally based on strategy
def player_turn(deck, player_hand, dealer_upcard, player):
    print("Player's turn...")
...
# Main game loop
player = Player()
deck = Deck()  # Deck should persist across rounds
while True:
    player_hand = Hand()
    dealer_hand = Hand()

    # Recalculate true count before placing bets
    ...
    # Print updated true count for debugging
    print(f"Updated True Count: {deck.true_count():.2f}")
